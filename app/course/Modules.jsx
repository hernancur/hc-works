"use client";

export default function Modules() {
  return (
    <div>
      {/* <h2 className="text-center text-4xl dark:text-neutral-400 uppercase mt-6 mb-6">
        MÓDULOS
      </h2> */}
      <div className="mx-auto text-center text-neutral-400 text-lg max-w-3xl mb-6">
        <h5
          className="text-center my-4 font-InterBold"
          style={{ color: "#116f91" }}
        >
          Módulo 1
        </h5>
        <p className=" text-center text-neutral-400 text-lg max-w-3xl">
          Fundamentos web, te familiarizarás con{" "}
          <span style={{ color: "#fd3a02" }}> HTML, CSS y JAVASCRIPT</span>. A
          medida que avancemos, profundizamos en cada uno de estos temas.
        </p>
        <h5
          className="text-center my-4 font-InterBold"
          style={{ color: "#116f91" }}
        >
          Módulo 2
        </h5>
        <p className=" text-center text-neutral-400 text-lg max-w-3xl">
          <span style={{ color: "#fd3a02" }}>
            JAVASCRIPT avanzado y Pensamiento lógico
          </span>
          . Vamos a entrar mucho más en el lenguaje de programación, serás capaz
          de resolver problemas difíciles usando Javascript y crear programas
          que resuelvan una tarea compleja por ti.
        </p>
        <h5
          className="text-center my-4 font-InterBold"
          style={{ color: "#116f91" }}
        >
          Módulo 3
        </h5>
        <p className=" text-center text-neutral-400 text-lg max-w-3xl">
          <span style={{ color: "#fd3a02" }}>
            HTML avanzado, estructura y optimización
          </span>
          . Este tercer módulo se centra en las diferentes habilidades que nos
          da HTML al momento de construir una página.
        </p>
        <h5
          className="text-center my-4 font-InterBold"
          style={{ color: "#116f91" }}
        >
          Módulo 4
        </h5>
        <p className=" text-center text-neutral-400 text-lg max-w-3xl mb-12">
          <span style={{ color: "#fd3a02" }}>
            CSS avanzado, diseño y experiencia de usuario
          </span>
          . En el último módulo antes de certificarte, aprenderás como hacer que
          tu página 'se vea como tu quieras y sea amigable para quienes la
          usen'.
        </p>
        <p className="mb-12" style={{ color: "#fd3a02" }}>
          Para obtener el CERTIFICADO es necesaria la entrega del proyecto que
          hacemos durante la cursada como 'tarea'.
        </p>
      </div>
    </div>
  );
}
